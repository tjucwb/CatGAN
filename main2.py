...
def main(arg):
    ...
    if arg.semi:
        ...
        semi_labels = []
        for i in range(100):
            semi_img, semi_label = train_set[i]
            if semi_imgs is None:
                semi_imgs = semi_img.unsqueeze(0)
            else:
                semi_imgs = torch.cat((semi_imgs,semi_img.unsqueeze(0)),dim=0)
            semi_labels.append(semi_label)
    ...
    for epoch in range(arg.epoch_num):
        for it, (r_img,label) in enumerate(train_loader):
            ...
            if arg.semi:
                ...
                semi_labels = torch.tensor(semi_labels,dtype=torch.long)
                if gpu_use:
                    semi_imgs = Variable(semi_imgs.cuda())
                    semi_labels = semi_labels.cuda()
                pre_label = d_net(semi_imgs)

                l_d = l_d + criterion(pre_label,semi_labels)
...
if __name__ == '__main__':
    ...
    arg.add_argument('--semi',action='store_true',help='determine if it is for semi-supervision')
